...
/*   Updated: 2024/11/08 02:15:58 by gholloco         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	exec_current(t_data *data, t_cmd *tmp, int *pid)
{
	if (tmp->args && tmp->args[0] && !ft_strncmp("exit", tmp->args[0], 5))
		...
		return (data->exit_code = ft_cd(&(data->env), tmp->args + 1),
			close_both_pid(pid[0], pid[1]));
	else if (tmp->args && tmp->args[0]
		&& !ft_strncmp("export", tmp->args[0], 7))
		return (data->exit_code = ft_export(&(data->env),
				tmp->args + 1, data->exit_code),
			close_both_pid(pid[0], pid[1]));
	else if (tmp->args && tmp->args[0] && !ft_strncmp("unset", tmp->args[0], 6))
		return (data->exit_code = ft_unset(&(data->env), tmp->args + 1),
			close_both_pid(pid[0], pid[1]));
	g_signal_pid = fork();
	if (g_signal_pid < 0)
